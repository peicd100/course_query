...
class MainWindow(QMainWindow):
    ...
    def _refresh_favorites_table(self) -> None:
        sorting = self.tbl_fav.isSortingEnabled()
        self.tbl_fav.setSortingEnabled(False)
        
        # 在清空與重建表格時暫時禁用拖曳，防止拖曳事件干擾
        ...
        # 最後才啟用排序並應用當前設定的排序方式
        self.tbl_fav.setSortingEnabled(False)  # 暫時禁用排序
        self.tbl_fav.horizontalHeader().setSortIndicator(self._fav_sort_section, self._fav_sort_order)
        if sorting:
            self.tbl_fav.setSortingEnabled(True)  # 然後啟用排序
            # 排序立即執行
